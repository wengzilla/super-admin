...
SuperAdmin is a **Rails admin framework** that replaces Administrate's server-rendered ERB views with a **React single-page application** powered by [Superglue](https://github.com/thoughtbot/superglue).

## Why SuperAdmin?

Administrate is great, but its ERB-based views can feel limiting when you want rich, interactive admin interfaces. SuperAdmin keeps the parts that work well — the dashboard DSL — and swaps the frontend for a modern React SPA.
...
SuperAdmin is two packages:

1. **`super_admin` gem** — Provides the dashboard DSL, field types, generic `.json.props` templates, controllers, and Rails generators.
2. **`super-admin` npm package** — A slim runtime with utilities (`cn`, `truncate`, `formatDate`), hooks (`useIsMobile`), and the field registry API.
