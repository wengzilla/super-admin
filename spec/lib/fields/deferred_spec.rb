...
RSpec.describe SuperAdmin::Field::Deferred do
  let(:deferred) { SuperAdmin::Field::String.with_options(truncate: 10) }

  describe "delegation" do
    it "delegates searchable? to wrapped field class" do
      expect(deferred.searchable?).to eq(SuperAdmin::Field::String.searchable?)
    end

    it "delegates sortable? to wrapped field class" do
      expect(deferred.sortable?).to eq(SuperAdmin::Field::String.sortable?)
    end

    it "delegates eager_load? to wrapped field class" do
      expect(deferred.eager_load?).to eq(SuperAdmin::Field::String.eager_load?)
    end

    it "delegates associative? to wrapped field class" do
      expect(deferred.associative?).to eq(SuperAdmin::Field::String.associative?)
    end
